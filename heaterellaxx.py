...
<div class="instruction-box">
    ...
    <p style="margin-bottom:0;">
        ...
        3. Inspect <strong>LMTD</strong>, <strong>Heat Duty (Q)</strong>, and <strong>Required Surface Area (A)</strong> metrics.<br>
...
if not valid:
    st.error(error_msg)
    st.info("💡 **Troubleshooting Guidance:** Adjust the temperature sliders/inputs in the sidebar so that $T_{h,in} > T_{h,out}$ and $T_{c,in} < T_{h,in}$.")
    st.stop()
...
with chart_col1:
    ...
    # Generate 100 points along normalized length (0 to 1)
    x = np.linspace(0, 1, 100)
    
    # Linear or exponential temperature approximation along length
    # Hot fluid cools from T_h_in at x=0 to T_h_out at x=1
    T_h_profile = T_h_in - (T_h_in - T_h_out) * x
    
    if flow_config == "Counterflow":
        # Cold fluid enters at x=1 (T_c_in) and exits at x=0 (T_c_out)
        T_c_profile = T_c_out - (T_c_out - T_c_in) * x
    else: # Parallel Flow
        # Cold fluid enters at x=0 (T_c_in) and exits at x=1 (T_c_out)
...
with chart_col2:
    ...
    # Calculate parallel vs counterflow benchmark for identical thermal loads
    # Counterflow
...
# ------------------------------------------------------------------------------
# 7. AI HEAT ASSISTANT & ENGINEERING INTERPRETATION
# ------------------------------------------------------------------------------
with st.expander("🤖 AI Engineering Interpretation & Decision Support", expanded=True):
    ...
    - **Pinch Temperature Analysis:** The minimum temperature difference ($\Delta T_{{min}}$) in your heat exchanger is **{min_dT:.2f}°C**.
      {'⚠️ *Warning: Pinch point is below 5°C, requiring large surface area or high fluid velocities to prevent thermal stalling.*' if min_dT < 5 else '✅ *Pinch point provides healthy temperature driving force across the exchanger length.*'}
    
    - **Flow Configuration Efficiency:** 
      - Current mode: **{flow_config}**.
      - Counterflow provides **{(cf_lmtd - pf_lmtd)/pf_lmtd*100:.1f}% higher LMTD** than Parallel flow for this thermal duty, reducing required surface area by **{((pf_area - cf_area)/pf_area*100 if pf_area > 0 else 0):.1f}%**.
